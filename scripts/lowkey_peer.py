...
if __name__ == '__main__':
    logger = Logger("lowkey_peer")
    master = Node()
    # master.import_key("validator_key")
    master.debug()

    protocol = UDPProtocol(master.node_event_handler, 2710, logger)
    ...
    while True:
        # mint to self
        logger.log("[bold blue]Minting...[/]")
        master.mint(w.address, mint_key, pmint_key)
        master.debug()

        logger.log("[bold blue]--------------------------------Waiting for confirm transaction...[/]")
        ...
        while len(master.node_event_handler.node.blockchain.mempool) == currentMempool:
            time.sleep(1)
        logger.log("[bold green]--------------------------------Done. Transaction confirmed.[/]")
        master.debug()
        time.sleep(3)