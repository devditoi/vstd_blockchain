...
class Validator:
    ...
    def validate_transaction_with_signature(tx: Transaction, signature: str, publicKey) -> bool:
        ...
        if not SignerFactory().get_signer().verify(tx.to_verifiable_string(), signature, publicKey):
            print("[bold red]validator.py:validate_transaction_with_signature:[/] Transaction signature is invalid")
        ...
        if not SignerFactory().get_signer().address(publicKey) == tx.sender and tx.Txtype != "mintburn":
            print("[bold red]validator.py:validate_transaction_with_signature:[/] Transaction sender is invalid")
    ...
    def validate_transaction_with_worldstate(tx: Transaction, worldState: WorldState) -> bool:
        ...
        if tx.gas_limit < ChainConfig.MinimumGasPrice:
            print("[bold red]Validator.py:offchain_validate:[/] Transaction gasPrice is below minimum")
            return False

        if worldState.get_eoa(tx.sender).balance < tx.gas_limit and tx.Txtype != "mintburn":
            print("[bold red]Validator.py:offchain_validate:[/] Transaction sender does not have enough balance")
            return False

        if tx.transactionData["amount"] <= 0:
            print("[bold red]Validator.py:offchain_validate:[/] Transaction amount is negative")
            return False
    ...
    def validate_transaction_raw(tx, pre_nonce_check = None):
        ...
        # Yeah wtf?
        if not tx.publicKey:
            print("[bold red]chain.py:process_block:[/] Transaction public key is missing")
            return False
        ...
        if not SignerFactory().get_signer().verify(tx.to_verifiable_string(), tx.signature, temp_public_key):
            print("[bold red]chain.py:process_block:[/] Transaction signature is invalid")
            return False

        if not SignerFactory().get_signer().address(temp_public_key) == tx.sender:
            print("[bold red]chain.py:process_block:[/] Transaction sender is invalid")
            return False
        ...
        if pre_nonce_check:
            if tx.sender in pre_nonce_check and tx.nonce != pre_nonce_check[tx.sender] + 1:
                print("[bold red]chain.py:process_block:[/] Transaction nonce is not valid")
                return False
    ...
    def validate_block_on_chain(block: Block, chain, initially=False) -> bool:
        if block.index != chain.get_height() and not initially:
            print(f"[bold red]chain.py:add_block:[/] Block index does not match chain length of {chain.get_height()}")
            print(f"Block index: {block.index}, Chain height: {chain.get_height()}")
            return False

        if block.previous_hash != chain.get_latest_block().hash:
            print("[bold red]chain.py:add_block:[/] Block previous hash does not match last block hash")
            return False

        if block.hash == chain.get_latest_block().hash:
            print("[bold red]chain.py:add_block:[/] Block hash already exists")
            return False
    ...
    def validate_receipts(block: Block, receipts: list[Transaction]) -> bool:
        ...
        if block.receipts_root != receipts_root:
            print("[bold red]chain.py:validate_receipts:[/] Receipts root hash does not match")
            print(f"Block receipts root: {block.receipts_root}, Calculated receipts root: {receipts_root}")
            return False
        return True