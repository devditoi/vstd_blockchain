...
class GetBlocksEvent(EventHandler):
    ...
    def handle(self, event: "NodeEvent"):
        ...
        logger.info(f"[{self.neh.node.origin}] GetBlocksEvent.handle: receiving request for blocks from {start_index} to {end_index} from {event.origin}")

        if end_index > self.neh.node.blockchain.get_height():
            logger.warning(f"[{self.neh.node.origin}] GetBlocksEvent.handle: end_index {end_index} is greater than current blockchain height {self.neh.node.blockchain.get_height()}. Adjusting to height.")
            return False
        ...
        logger.info(f"[{self.neh.node.origin}] GetBlocksEvent.handle: sending back {len(blocks)} blocks to {event.origin}")
...
class BlocksEvent(EventHandler):
    ...
    def handle(self, event: "NodeEvent"):
        ...
        logger.info(f"[{self.neh.node.origin}] BlocksEvent.handle: received {len(blocks_data)} blocks from {event.origin}")
        ...
        if len(blocks_data) > 1:
            ...
            if highest_point < self.neh.node.blockchain.get_height():
                logger.warning("[BlocksEvent.handle] Received block with lower index than current blockchain height, Not implemented yet")
                return False
        ...
        for block_data in blocks_data:
            ...
            logger.info(f"[{self.neh.node.origin}] BlocksEvent.handle: adding block {block.index} to blockchain")
